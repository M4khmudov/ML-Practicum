...
def prepare(config_path=None):
    ...
    # data.yaml для обучения YOLOv8/YOLOv5 через ultralytics (см. train.py)
    class_names = "\n".join(f'  - "{i}"' for i in range(43))
    data_yaml_content = (
    ...
    (yolo_root / "data.yaml").write_text(data_yaml_content, encoding="utf-8")
    log.info(f"data.yaml сохранён -> {(yolo_root / 'data.yaml').resolve()}")
